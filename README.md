...
| ファイル種別 | ファイル数 | 行数 | 説明 |
|------------|----------|------|------|
| C# (.cs) | 121 | 10,246 | アプリケーションコード |
| Razor (.razor) | 17 | 2,416 | Blazor UIコンポーネント |
| プロジェクトファイル (.csproj) | 20 | 364 | プロジェクト定義 |
| JSON (.json) | 18 | 277 | 設定ファイル |
| CSS (.css) | 3 | 262 | カスタムスタイル |
| Markdown (.md) | 4 | 703 | ドキュメント |
| **合計** | **183** | **14,268** | ソースコード（サードパーティライブラリ除く） |
...
| レイヤー | 行数 | 説明 |
|---------|------|------|
| Domain層 | 1,119 | ビジネスロジック・エンティティ |
| Application層 | 1,094 | ユースケース・サービス |
| Infrastructure層 | 3,860 | データアクセス・外部連携 |
| API層 | 1,784 | Web APIエンドポイント |
| UI層（Blazor） | 3,904 | フロントエンドUI |
| 共有ライブラリ | 859 | 共通コンポーネント |
| AppHost | 42 | Aspireオーケストレーション |
